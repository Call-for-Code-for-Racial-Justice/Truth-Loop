...
const getOfficials = (callback) => {
  pool.query(
    'SELECT * FROM official ORDER BY id ASC',
    callback
  )
}
...
const getOfficialById = (id, callback) => {
  pool.query(
    'SELECT * FROM official WHERE id=$1',
    [id],
    callback
  )
}
...
const createOfficial = (data, callback) => {
  pool.query(
    ...
    [data.name, data.title, data.email_address, data.phone_number, data.website_url],
    callback
  )
}
...
const updateOfficial = (id, data, callback) => {
  pool.query(
    ...
    [data.name, data.title, data.email_address, data.phone_number, data.website_url, id],
    callback
  )
}
...
const deleteOfficial = (id, callback) => {
  pool.query(
    'DELETE FROM official WHERE id=$1',
    [id],
    callback
  )
}
