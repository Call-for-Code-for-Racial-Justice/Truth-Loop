...
const RecordRoute = () => {
  ...
  const videoDisplayRef = useRef({ muted: false, controls: true, autoplay: false })
  ...
  const onUserMediaRetrievalSuccess = useCallback(streamFromMedia => {
    const options = {
  ...
  const onRecordingStopped = useCallback(audioVideoWebmUrl => {
  ...
  const renderWebcam = () => {
    return <><Webcam ref={webcamRef} width={'100%'}
                   data-testid={'webcam'}
  ...
  const renderVideoPlayer = () => {
    return (<div data-testid={'videoPlayer'}>
      <video ref={videoDisplayRef} className="video" controls src={videoPlayerSource}/>
    </div>)
  }
        ...
        <div className="bx--col r3-col">
          <Grid className={'video-control-panel'}>
            ...
            <Row className={'grid-row'}>
              ...
              <Column className={'col-2'}>
                {recording ? renderStopRecordingOption() : renderStartRecordingOption()}
              </Column>
