    ...
    <v-card class="maincard">
      ...
      >
        ...
        <template v-slot:item.website_url="{ item }">
          <v-btn small icon :disabled="!isUrl(item.website_url)" :href="item.website_url">
...
import AdvocacyGroupsForm from '../components/AdvocacyGroupsForm.vue';

export default {
  name: 'advocacy-groups',
  components: {
    'edit-form': AdvocacyGroupsForm,
  },
  data() {
    return {
      items: [],
  ...
  methods: {
    ...
    async deleteItem(id) {
      try {
        await fetch(`/api/v1/advocacyGroups/${id}`, {
          method: 'DELETE',
        });
      } catch (error) {
        console.error(error);
