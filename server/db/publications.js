...
const getPublications = (callback) => {
  pool.query('SELECT * FROM publication ORDER BY id ASC', callback)
}

const getPublicationById = (id, callback) => {
  pool.query('SELECT * FROM publication WHERE id = $1', [id], callback)
}
...
const deletePublication = (id, callback) => {
  pool.query('DELETE FROM publication WHERE id = $1', [id], callback)
}
