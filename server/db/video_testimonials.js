...
const getVideoTestimonials = (callback) => {
  pool.query('SELECT  * FROM video_testimonial ORDER BY id ASC', callback)
}

const getVideoTestimonialById = (id, callback) => {
  pool.query('SELECT * FROM video_testimonial WHERE id = $1', [id], callback)
}
...
const deleteVideoTestimonial = (id, callback) => {
  pool.query('DELETE FROM video_testimonial WHERE id=$1', [id], callback)
}
