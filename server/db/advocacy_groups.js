...
const getAdvocacyGroups = (callback) => {
  pool.query('SELECT * FROM advocacy_group ORDER BY id ASC', callback)
}

const getAdvocacyGroupById = (id, callback) => {
  pool.query('SELECT * FROM advocacy_group WHERE id=$1', [id], callback)
}
...
const deleteAdvocacyGroup = (id, callback) => {
  pool.query('DELETE FROM advocacy_group WHERE id=$1', [id], callback)
}
