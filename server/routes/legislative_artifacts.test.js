...
describe('CRUD TESTS FOR /api/v1/legislativeArtifacts', function () {
  ...
  it('GET should not have the item.', async done => {
    let res = await request(app)
      .get('/api/v1/legislativeArtifacts').set('Accept', 'application/json').expect('Content-Type', /json/)
  ...
  it('GET minDetail should now include the item', async done => {
    let res = await request(app)
      .get('/api/v1/legislativeArtifacts/list/minDetail').set('Accept', 'application/json').expect('Content-Type', /json/)
    expect(res.status).toBe(200);
