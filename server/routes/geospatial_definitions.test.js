...
describe('CRUD TESTS FOR /api/v1/geospatialDefinitions', function () {

  let payload = {
    name: `Test Name Date ${Date.now()} Randy ${Math.random()}`,
    short_name_ui: `Test ShortNameUI Date ${Date.now()} Randy ${Math.random()}`,
  ...
  it('GET should not have the item.', async done => {
    let res = await request(app)
      .get('/api/v1/geospatialDefinitions').set('Accept', 'application/json').expect('Content-Type', /json/)
  ...
  it('GET by NAME should return the same item', async done => {
    let res = await request(app)
      .get(`/api/v1/geospatialDefinitions/name/${payload.name}`).set('Accept', 'application/json').expect('Content-Type', /json/)
    expect(res.status).toBe(200);
    expect(res.body).toEqual(expect.objectContaining(payload));
    itemByName = res.body;
  ...
  it('PUT by ID to change the description', async done => {
    ...
    let res = await request(app)
      .put(`/api/v1/geospatialDefinitions/${id}`)
      .set('Accept', 'application/json')
      .send(updatedPayload)
      .expect('Content-Type', /json/)
      .expect(200);
