...
const REFRESH_TOKEN_SECRET = fs.readFileSync(
  path.join(__dirname, '..', 'secrets', 'REFRESH_TOKEN_SECRET.txt'),
  'utf8'
)
                    ...
                    if (error) {
                      ...
                      generateRefreshToken(
                        ...
                        function (error, result) {
                          if (error) {
                            logger.error(
                              'failed to generate refresh token: %s',
                              error
                            )
                            res.status(500).json({
                              error: 'Internal Server Error',
                            ...
                            Token.addRefreshToken(
                              ...
                              function (error, results) {
                                if (error) {
                                  logger.error(
                                    'failed to save refresh token: %s',
                                    error
                                  )
                                  res.status(500).json({
                                    error: 'Internal Server Error',
              ...
              if (results.rows.length == 0) {
                ...
                if (results.rows[0].status == 'valid') {
                  ...
                  res
                    .status(301)
                    .send(
                      'The refresh token has been revoked please log in to issue a new one.'
                    )
                }
              }
            }
