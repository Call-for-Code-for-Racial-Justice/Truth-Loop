...
describe('CRUD TESTS FOR /api/v1/levels', function () {
  ...
  it('GET should not have the item.', async done => {
    let res = await request(app)
      .get('/api/v1/levels').set('Accept', 'application/json').expect('Content-Type', /json/)
  ...
  it('GET by NAME should return the same item', async done => {
    let res = await request(app)
      .get(`/api/v1/levels/name/${payload.name}`).set('Accept', 'application/json').expect('Content-Type', /json/)
    expect(res.status).toBe(200);
    expect(res.body).toEqual(expect.objectContaining(payload));
    itemByName = res.body;
