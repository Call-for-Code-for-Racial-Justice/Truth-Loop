		...
		<div data-testid={'policyTable'}>
			<Grid className={'policy-table'}>
				<Row className={'policy-title-row'}>
					<Column>{`POLICIES: (${filteredItems.length > 0 ? filteredItems.length : policies.length})`}</Column>
				</Row>
				{filteredItems.length > 0 ? filteredItems.map(renderItem) : policies.map(renderItem)}
			</Grid>
		</div>
	)
}
