...
referred as either **Legislative Artifacts** or **PR&L**) impacts them or
...
This solution combines use of a media server (currently Watson Media) and data storage to hold the curated legislative artifacts, and the meta-data to link these together.

- The User launches the web app (on either a mobile or laptop/desktop device) and can view the range of curated legislative artifacts that have been created (1). The Vue app retrieves these by sending a REST request to the API server, which extracts them (3) from the SQL database.
